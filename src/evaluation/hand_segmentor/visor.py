"""Evaluate the reusable hand segmentor on EPIC-KITCHENS VISOR."""
...
from datasets.epic_kitchens.visor import DEFAULT_DATA_ROOT, list_visor_items, load_visor_sample
...
DEFAULT_OUTPUT = ROOT / "outputs/evaluation/hand_segmentor/visor.json"
...
def main() -> None:
    ...
    items = list_visor_items(splits, data_root=args.data_root, max_frames=args.max_frames)
    if not items:
        raise RuntimeError(f"No VISOR hand-evaluation frames found for splits: {splits}")
    write_dir = args.output.parent / "visor_masks" if args.write_masks else None
